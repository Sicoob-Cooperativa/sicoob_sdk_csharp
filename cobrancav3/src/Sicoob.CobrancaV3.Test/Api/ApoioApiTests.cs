...
{
    /// <summary>
    ///  Class for testing ApoioApi
    /// </summary>
    public sealed class ApoioApiTests : ApiTestsBase
    {
        private readonly IApoioApi _instance;

        public ApoioApiTests(): base(Array.Empty<string>())
        {
            _instance = _host.Services.GetRequiredService<IApoioApi>();
        }
        ...
        {
            var response = await _instance.HealthGetAsync();
            var model = response.Ok();
            Assert.IsType<HealthGet200Response>(model);
        }
    }
}
